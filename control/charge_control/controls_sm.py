...
def _update_mid(current: int, a: int, b: int):
    lower = min(a, b)
    upper = max(a, b)
...
def _update_mid_max(current: int, a: int, b: int):
    lower = min(a, b)
    upper = max(a, b)
    changed = current is None or current > upper
    if changed:
        current = (a + b) // 2
    else:
...
class ControlsSM:
    ...
    def update(self, a: Controls, b: Controls) -> Controls:
        wallbox_current, wb_changed = _update_mid(self._controls.wallbox_current,
                                                  a.wallbox_current,
                                                  b.wallbox_current)
        battery_max_discharge, d_changed = _update_mid(self._controls.battery_max_discharge,
                                                       a.battery_max_discharge,
                                                       b.battery_max_discharge)
        battery_max_charge, c_changed = _update_mid_max(self._controls.battery_max_charge,
